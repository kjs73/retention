...
public:
	...
	,N(Lxin*Lyin)
	,N2(2*Lxin*Lyin)
	,NB(2*Lxin*Lyin) // number of bonds in periodic lattice, torus
	,NBF(2*Lxin*Lyin - Lyin - Lxin) // number of bonds in free lattice, stripe
	,L_2(Lxin*Lyin)
	...
	unsigned int NumberNeighbors()const{
		return 4;
	}
	bool left(const unsigned int& ste)const{
		return (ste%lx==0);
	}
	bool right(const unsigned int& ste)const{
		return (((ste+1)%lx)==0);
	}
	bool bottom(const unsigned int& ste)const{
		return ste<lx;
	}
	bool top(const unsigned int& ste)const{
		return ste>=TopConst;
	}
	unsigned int neighbor(const unsigned int& ste, const unsigned int& dir)const{
		...
		if (dir==0){
		    ...
		    throw std::runtime_error("lattice: illegal index");
		}
	}
...
#endif//LATTICE_STRIP_SQUARE_2D_HPP_KJS
