...
class parameter{
public:
    parameter(const std::string name__)
    :name_(name__)
    ,set_(false)
    {}
    ...
    parameter(const std::string name__, const T input)
    :name_(name__)
    ,set_(true)
    ,value_(input)
    {}
    ...
    T value()const{
        if (set()==false) throw std::runtime_error("parameter: illegal attempt to read empty value: name: "+name());
        return value_;
    }
